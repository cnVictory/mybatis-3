...
class PropertyParserTest {
    ...
    void replaceToVariableValue() {
        ...
        Assertions.assertThat(PropertyParser.parse("SELECT * FROM ${tableName:users} ORDER BY " +
                "${orderColumn:id}", props)).isEqualTo("SELECT * FROM members ORDER BY member_id");
    ...
    void applyDefaultValue() {
        ...
        Assertions.assertThat(PropertyParser.parse("SELECT * FROM ${tableName:users} ORDER BY " +
                "${orderColumn:id}", props)).isEqualTo("SELECT * FROM users ORDER BY id");
    ...
    void applyCustomSeparator() {
        ...
        Assertions.assertThat(PropertyParser.parse("${key?:default}", props)).isEqualTo("default");
        Assertions.assertThat(PropertyParser.parse("SELECT * FROM ${schema?:prod}.${tableName == null ? " +
                "'users' : tableName} ORDER BY ${orderColumn}", props)).isEqualTo("SELECT * FROM prod" +
                ".${tableName == null ? 'users' : tableName} ORDER BY ${orderColumn}");
        Assertions.assertThat(PropertyParser.parse("${key?:}", props)).isEmpty();
        Assertions.assertThat(PropertyParser.parse("${key?: }", props)).isEqualTo(" ");
        Assertions.assertThat(PropertyParser.parse("${key?::}", props)).isEqualTo(":");
    }
