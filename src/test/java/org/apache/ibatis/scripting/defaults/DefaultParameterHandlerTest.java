...
class DefaultParameterHandlerTest {
    ...
    void setParametersThrowsProperException() throws SQLException {
        ...
        doThrow(new SQLException("foo")).when(typeHandler).setParameter(any(PreparedStatement.class),
                anyInt(), any(), any(JdbcType.class));
        ParameterMapping parameterMapping = new ParameterMapping.Builder(mappedStatement.getConfiguration()
                , "prop", typeHandler).build();
        ...
        DefaultParameterHandler defaultParameterHandler = new DefaultParameterHandler(mappedStatement,
    ...
    MappedStatement getMappedStatement() {
        ...
        return new MappedStatement.Builder(config, "testSelect", new StaticSqlSource(config, "some select " +
                "statement"), SqlCommandType.SELECT).resultMaps(
