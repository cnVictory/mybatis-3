...
public interface SPMapper {
    ...
    // annotated
    @Select({"{call sptest.adder(", "#{addend1,jdbcType=INTEGER,mode=IN},", "#{addend2,jdbcType=INTEGER," +
            "mode=IN},", "#{sum,jdbcType=INTEGER,mode=OUT})}"})
    @Options(statementType = StatementType.CALLABLE)
    Object adderAsSelectAnnotated(Parameter parameter);

    @Update({"{call sptest.adder(", "#{addend1,jdbcType=INTEGER,mode=IN},", "#{addend2,jdbcType=INTEGER," +
            "mode=IN},", "#{sum,jdbcType=INTEGER,mode=OUT})}"})
    @Options(statementType = StatementType.CALLABLE)
    ...
    @Select({"{call sptest.getnames(", "#{lowestId,jdbcType=INTEGER,mode=IN},", "#{totalRows," +
    ...
    @Select({"{call sptest.getnamesLowHigh(", "#{lowestId,jdbcType=INTEGER,mode=IN},", "#{highestId," +
    ...
    @Select({"{call sptest.arraytest(", "#{ids,mode=IN,jdbcType=ARRAY},", "#{requestedRows," +
            "jdbcType=INTEGER,mode=OUT},", "#{returnedIds,mode=OUT,jdbcType=ARRAY})}"})
    @Results({@Result(column = "ID", property = "id"), @Result(column = "FIRST_NAME", property = "firstName"
    ), @Result(column = "LAST_NAME", property = "lastName")})
    @Options(statementType = StatementType.CALLABLE)
