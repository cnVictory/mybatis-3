...
class IncludePropertyTest {
    ...
    static void setUp() throws Exception {
        ...
        try (Reader reader = Resources.getResourceAsReader("org/apache/ibatis/submitted/include_property" +
                "/mybatis-config.xml")) {
    ...
    void testSimpleProperty() {
        try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
            List<String> results = sqlSession.selectList("org.apache.ibatis.submitted.include_property" +
                    ".Mapper.selectSimpleA");
            assertEquals("col_a value", results.get(0));
            results = sqlSession.selectList("org.apache.ibatis.submitted.include_property.Mapper" +
                    ".selectSimpleB");
    ...
    void testPropertyContext() {
        try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
            List<Map<String, String>> results = sqlSession.selectList("org.apache.ibatis.submitted" +
                    ".include_property.Mapper.selectPropertyContext");
    ...
    void testNestedDynamicValue() {
        try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
            List<String> results = sqlSession.selectList("org.apache.ibatis.submitted.include_property" +
                    ".Mapper.selectNestedDynamicValue");
            assertEquals("col_a value", results.get(0));
        }
    }
    ...
    void testRuntimeVar() {
        try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
            ...
            params.put("suffix", "b");
            List<String> results = sqlSession.selectList("org.apache.ibatis.submitted.include_property" +
                    ".Mapper.selectRuntimeVar", params);
            assertEquals("col_b value", results.get(0));
        }
    }
    ...
    void testParametersInAttribute() {
        try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
            List<Map<String, String>> results = sqlSession.selectList("org.apache.ibatis.submitted" +
                    ".include_property.Mapper.selectPropertyInAttribute");
            Map<String, String> map = results.get(0);
            assertEquals(2, map.size());
